...
func Migrate(db *gorm.DB) error {
	...
	slog.Info("database migrated completed")
	return nil
}

// SeedParts inserts sample part data into the database
// If the table already contains data, this function returns early
// This is useful for development and testing enviroments
func SeedParts(db *gorm.DB) error {
	...
	// Batch insertion is more efficient than inseting one by one
	if err := db.CreateInBatches(parts, 100).Error; err != nil {
		return fmt.Errorf("failed to seed parts: %w", err)
	}

	// Log succesful seeding with the number of parts inserted
