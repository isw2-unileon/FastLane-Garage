// Contains HTTP request handlers for the API endpoints
package handlers
...
// Returns a Gin handler function that fetches all parts form the database
...
func GetParts(db *gorm.DB) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		// Initialize an empty slice to store parts retrieved form the database
		...
		if err := db.Find(&parts).Error; err != nil {
			// If an error occurs
			// respond with HTTP 500 Interval Server Error
